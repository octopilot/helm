...
|----------|---------|----------------|
| `BP_HELM_OCI_REF` | OCI repository reference (no tag), typically set by the platform from `--repo` or `SKAFFOLD_DEFAULT_REPO`. The buildpack runs `helm push ... oci://${BP_HELM_OCI_REF}`; Helm tags the pushed chart with the chart version (e.g. `0.1.0`). Scheme (HTTP/HTTPS) is auto-detected unless `BP_HELM_OCI_PLAIN_HTTP` is set: tries HTTPS first (valid or self-signed), then HTTP. Works with ttl.sh, ghcr.io, and any OCI registry. | e.g. `ttl.sh/my-uuid-chart`, `ghcr.io/org/repo-chart`, `localhost:5001/org/repo-chart` |
